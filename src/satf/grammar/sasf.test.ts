...
const tests = [
  ...
  [
    ...
    $$EOD
        request(VFT2_REQUEST_01,
        START_TIME, 2024-266T19:59:57,FROM_PREVIOUS_START
        REQUESTOR, "vft2_anichola",
        PROCESSOR, "VC2AB",
        KEY, "No_Key")
    ...
    `satf_sasf(Sasf(Header,Body(
      Requests(
        Request(RequestName,Time,TimeRelation,Requestor,Processor,Key,
...
describe.each([["parse tree structure", tests]])(
  "grammar tests - %s",
  (_name: string, testArray: string[][]) => {
    ...
    undefined when they match. */
      expect(
        testTree(SatfLanguage.parser.parse(input), expected, undefined),
      ).toBeUndefined();
    });
  },
);
