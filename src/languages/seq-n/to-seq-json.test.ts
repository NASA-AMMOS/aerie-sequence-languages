...
HDW_CMD`;
    ...
    const actual = sequenceToSeqJson(SeqParser.parse(seq), seq, commandDictionary, id);
...
R71 ECHO    L02STR
    ...
    const actual = sequenceToSeqJson(SeqParser.parse(seq), seq, commandBanana, id);
    expect(actual).toEqual(expectedJson);
  });
...
C ECHO SIZE
  ...
  it('header ordering', async () => {
    ...
    for (const ordering of permutations) {
      ...
      const actual = sequenceToSeqJson(SeqParser.parse(input), input, commandBanana, 'id');
      const expected = {
        id: 'test.seq',
        locals: [
          {
...
@ENGINE -1
    `;
    const actual = sequenceToSeqJson(SeqParser.parse(seq), seq, commandBanana, '');
    const expectedJson = {
      id,
      metadata: {},
      steps: [
